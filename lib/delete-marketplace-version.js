...
module.exports = async (pluginConfig, context) => {
  const { version, logger } = context

  if (!version || !version.length) {
    debug('deleting product version failed. No input data available.')
    logger.error(
      'WHMCS Marketplace deleting product version failed. No input data available.'
    )
    return false
  }
  ...
  if (success) {
    try {
      ...
      logger.error(
        'WHMCS Marketplace deleting product version failed.',
        error.message
      )
      success = false
    }
